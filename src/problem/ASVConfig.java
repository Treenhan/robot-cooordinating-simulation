...
public class ASVConfig {
	...
	/**
	 * constructor using list of 2d Points
	 */
	public ASVConfig(List<Point2D> list){
		this.asvPositions = list;
	}
	...
	public PolarConfig convertToPolar(){
		...
		Point2D point = new Point2D.Double(0,0);//this is the first point in the sequence
		...
		for(int i=0;i<asvPositions.size()-1;i++){
			...
			xOffSet = endPoint.getX() - initPoint.getX();
			yOffSet = endPoint.getY() - initPoint.getY();

			if(xOffSet<0){
				if(yOffSet>=0){
					angle =  Math.PI - Math.abs(Math.atan(yOffSet/xOffSet));
				}else{
					angle =  - Math.PI + Math.abs(Math.atan(yOffSet/xOffSet));
				}
			}else angle = Math.atan(yOffSet/xOffSet);

			angles.add(angle);


		}
		PolarConfig polarConfig = new PolarConfig(point,angles);
